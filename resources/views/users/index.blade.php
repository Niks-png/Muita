

<x-app-layout>
    <div class="py-8">
        <div class="max-w-5xl mx-auto px-4">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-white">Users</h2>
                <a href="{{ route('users.create') }}" class=" hover:bg-gray-700 text-white px-4 py-2 rounded">
                    + Add User
                </a>
            </div>

            <div class="space-y-4">
                @foreach ($users as $user)
                <div class="border border-gray-300 p-6 rounded text-white flex justify-between items-center">
                    <div class="flex-1">
                        <h4 class="text-lg font-semibold">{{ $user->full_name }}</h4>
                        <p class="text-gray-400 text-sm">{{ $user->username }}</p>
                        <span class="inline-block mt-2 px-3 py-1 text-xs font-medium rounded ">
                            {{ ucfirst($user->role) }}
                        </span>
                    </div>
                    <div class="flex gap-2">
                        
                       
                    </div>
                    <a href="{{ route('users.show', $user->id) }}" class=" hover:bg-gray-700 px-4 py-2 rounded text-sm">
                            View
                        </a>
                </div>
                @endforeach
            </div>
        </div>
    </div>
</x-app-layout>